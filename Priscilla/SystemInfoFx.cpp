						...
						while (pEnumCOMDevs && SUCCEEDED(pEnumCOMDevs->Next(10000, 1, &pCOMDev, &uReturned)) && uReturned == 1)
						{
							...
							*/
							...
							if (speed > 0 && *cores > 0 && *threads > 0)
							{
								...
								{
									cpuInfo.Format(_T("%s %dMHz (%dcores/%dthreads)"), name.GetString(), speed, *cores, *threads);
								}
								else if (*cores == 1 && *threads == 1)
								{
									cpuInfo.Format(_T("%s %dMHz (%dcore/%dthread)"), name.GetString(), speed, *cores, *threads);
								}
								else if (*cores == 1)
								{
									cpuInfo.Format(_T("%s %dMHz (%dcore/%dthreads)"), name.GetString(), speed, *cores, *threads);
								}
							}
							else if (*cores > 0 && *threads > 0)
							{
								if (*cores > 1 && *threads > 1)
								{
									cpuInfo.Format(_T("%s (%dcores/%dthreads)"), name.GetString(), *cores, *threads);
								}
								else if (*cores == 1 && *threads == 1)
								{
									cpuInfo.Format(_T("%s (%dcore/%dthread)"), name.GetString(), *cores, *threads);
								}
								else if (*cores == 1)
								{
									cpuInfo.Format(_T("%s (%dcore/%dthreads)"), name.GetString(), *cores, *threads);
								}
							}
							else
							{
	...
	if (cpuInfo.IsEmpty())
	{
		...
		if (! IsWin9x())
		{
			if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0"), 0, KEY_READ, &hKey) == ERROR_SUCCESS)
			{
				...
				{
					cpuInfo.Format(_T("%s %dMHz"), cpuInfo.GetString(), value);
				}
				RegCloseKey(hKey);
			}
		}
...
void GetMemoryInfo(CString& memoryInfo)
{
#if _MSC_VER > 1310
	...
	{
		memoryInfo.Format(_T("%d GB"), (int)(memory.ullTotalPhys / 1024.0 / 1024 / 1024 + 1));
	...
	{
		memoryInfo.Format(_T("%d GB"), (int)(memory.dwTotalPhys / 1024.0 / 1024 / 1024 + 1));
